...
`Mode` | QP adjustment mode for ROIs. Defines `DeltaQP` or absolute value is used during encoding. if `MFX_MBQP_MODE_QP_ADAPTIVE` is set mode could be adjusted for each block.
`BlockSize` | Granularity of QP map, valid values 8(HEVC only), 16, 32, 64
`NumQPAlloc` | The allocated QP array size.
`QP` | Pointer to a list of per-macroblock QP in raster scan order. In case of interlaced encoding the first half of QP array affects top field and the second – bottom field.<br><br>For AVC/HEVC valid range is 1..51.<br><br>Application’s provided QP values should be valid; otherwise invalid QP values may cause undefined behavior. MBQP map should be aligned for `BlockSize` <br><br>For MPEG2 QP corresponds to quantizer_scale of the ISO*/IEC* 13818-2 specification and have valid range 1..112.
`DeltaQP` | Pointer to a list of per-macroblock delta QPs in raster scan order. In case of interlaced encoding the first half of QP array affects top field and the second – bottom field.<br/><br>For AVC/HEVC valid range is dependent to HW capabilities, for GEN it is -51..51.<br/><br>Application’s provided delta QP values should be valid; otherwise invalid QP values may cause undefined behavior. MBQP map should be aligned for `BlockSize`<br>
`mfxQPandMode` | Pointer to a list of per block values of `Mode` and `QP`/`DeltaQP` in raster scan order. Adaptive mode is used when `mode` == `MFX_MBQP_MODE_QP_ADAPTIVE`, allows application to set QP or delta QP for each block. Refer to `QP` and `DeltaQP` about semantics of values.

**Change History**

This structure is available since SDK API 1.13.

The SDK API 1.34 adds new mode `MFX_MBQP_MODE_QP_ADAPTIVE`  and structure `mfxQPandMode` - ability to set `QP` or `DeltaQP` adaptively for each block.
...
| `Area`                     | Pointer to an array of IPCM regions. Different regions may overlap each other. |      |      |
...
`MFX_FOURCC_IYUV` | Same as I420.
`MFX_FOURCC_I010` | 10 bit per sample YUV 4:2:0 planar, each component has its own plane.
**Change History**
...
SDK API 1.34 adds `MFX_EXTBUFF_ENCODER_IPCM_AREA` and `MFX_EXTBUFF_INSERT_HEADERS`
