...
#if defined(__i386__)
    #define LIBMFXSW "libvplswref32.so.2"
    #define LIBMFXHW "libmfxhw32.so.2"
#elif defined(__x86_64__)
    #define LIBMFXSW "libvplswref64.so.2"
    #define LIBMFXHW "libmfxhw64.so.2"
...
mfxStatus LoaderCtx::Init(mfxInitParam& par, char* dllName) {
    ...
    if (dllName) {
        ...
        libs.emplace_back(MFX_MODULES_DIR "/" LIBMFXSW);
             ...
             par.Implementation & MFX_IMPL_VIA_HDDL) {
        libs.emplace_back(LIBMFXHW);
        libs.emplace_back(MFX_MODULES_DIR "/" LIBMFXHW);
    }
